...
public class LZViewPager: UIView {
    ...
    private var headerLeading: CGFloat {
        if let leading = dataSource?.leftMarginForHeader?() {
            return leading
        } else {
            return 0
        }
    ...
    private var headerTrailing: CGFloat {
        if let trailing = dataSource?.rightMarginForHeader?() {
            return trailing
        } else {
            return 0
        }
    ...
    private var defaultPageIndex: Int? {
        guard let itemsCount = dataSource?.numberOfItems() else {
            return nil
        }
        for i in 0 ..< itemsCount {
            guard let button = dataSource?.button(at: i) else {
                continue
            }
    ...
    public func reload() {
        ...
        if let showSeparator = dataSource?.shouldShowSeparator?(), showSeparator {
        ...
        headerView.pagerDelegate = delegate
        headerView.dataSource = dataSource
        contentView.delegate = delegate
        contentView.dataSource = dataSource
        contentView.hostController = hostController
        layoutIfNeeded()
        headerView.reload()
        contentView.reload()
    }
    ...
    public func select(index: Int, animated: Bool = true) {
        guard let itemsCount = dataSource?.numberOfItems(), index < itemsCount else {
